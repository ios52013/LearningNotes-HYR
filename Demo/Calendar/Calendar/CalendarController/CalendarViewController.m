...
- (void)requestSchedules {
    ...
    //NSLog(@"请求会议排期的参数：%@", scheduleParameter);
    ...
    //3.发送请求
    [getScheduleNetwork requestWithURLStr:url andNetworkInterface:NetworkInterfaceSchedule andHttpMethod:@"POST" andParameter:parameterDict];
...
- (void)viewDidLoad {
    ...
    [self relayoutForInterfaceOrientation:[self interfaceOrientation] withDuration:0.3];
}
...
- (void)monthChanged:(CFGregorianDate)currentCalendarDate {
    ...
    NSArray *tempArray = (NSArray *)[Schedule arrayFromSqlite];
